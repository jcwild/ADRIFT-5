    ...
    Friend ReadOnly Property Name(Optional ByVal Pronoun As PronounEnum = PronounEnum.Subjective, Optional bMarkAsSeen As Boolean = True, Optional ByVal bAllowPronouns As Boolean = True, Optional ByVal bForceTheName As Boolean = False) As String
        Get
            With UserSession
                ...
                Dim bReplaceWithPronoun As Boolean = False
                Dim bTheName As Boolean = bForceTheName
                ...
                If .bDisplaying Then
                    ...
                    If .PronounKeys.Count > 0 Then
                        ...
                        For i As Integer = .PronounKeys.Count - 1 To 0 Step -1
                            With .PronounKeys(i)
                                If .Gender = Gender AndAlso .Key = Key Then ' AndAlso .Pronoun = Pronoun 
    ...
    Public ReadOnly Property Descriptor(Optional ByVal TheName As Boolean = False) As String
        Get
            If arlDescriptors.Count > 0 Then
                Dim sArticle2 As String = sArticle
...
#If Runner Then
    ...
    Public Sub IncrementTimer()
        ...
        If TimerToEndOfWalk > 0 AndAlso TimerFromStartOfWalk > 0 Then
            DoAnySteps()
            DoAnySubWalks()
        End If
                    ...
                    If sDestination = HIDDEN OrElse Adventure.htblLocations.ContainsKey(sDestination) Then
                        With Adventure.htblCharacters(sKey)
                            If .HasProperty("ShowEnterExit") AndAlso .Location.ExistWhere = clsCharacterLocation.ExistsWhereEnum.AtLocation Then
                                If .Location.Key = Adventure.Player.Location.Key Then
                                    ...
                                    If Adventure.htblLocations(Adventure.Player.Location.Key).IsAdjacent(sDestination) Then
                                        Dim sTheDirection As String = Adventure.htblLocations(.Location.Key).DirectionTo(sDestination)
                                ...
                                ElseIf sDestination = Adventure.Player.Location.Key Then
                                    ...
                                    If Adventure.htblLocations(Adventure.Player.Location.Key).IsAdjacent(.Location.Key) Then
                                        Dim sTheDirection As String = Adventure.htblLocations(sDestination).DirectionTo(.Location.Key)
